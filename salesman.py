...
def main():
    ...
    for i in range(n):
        row = list(map(int, input().split()))                       #split converts input string into list of strings . map converts each string into integer . 
        tsp.append(row)

    solution, length = hillClimbing(tsp)

    print("Best Route Found:", solution)
    print("Minimum Distance:", length)

def hillClimbing(tsp):                                                          # tjos function tries to find a better solution step by step until no better solution can be found . 
...
def getNeighbours(currentSolution):                                             # swapping positions . 
    neighbours = []
    for i in range(len(currentSolution)):
